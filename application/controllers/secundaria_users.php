<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	class Secundaria_users extends CI_Controller {
		...
		public function useredit () {
			if ( $this->input->is_ajax_request() ) {
				if ( $this->session->userdata('cas-secundaria-userid') && $this->session->userdata('cas-secundaria-username') && $this->session->userdata('cas-secundaria-userdashboard') ) {
					...
					$useredit_info = $this->secundaria_users_mdl->get_userData_by_ID($this->session->userdata('cas-secundaria-userid'));
					$useredit_data = array(
					...
					$this->load->view('secundaria/window/window-user-edit-profile', $useredit_data);
		...
		public function useredit_attempt () {
			if ( $this->input->is_ajax_request() ) {
				if ( $this->session->userdata('cas-secundaria-userid') && $this->session->userdata('cas-secundaria-username') && $this->session->userdata('cas-secundaria-userdashboard') ) {
					...
					$this->load->view('secundaria/window/window-user-edit-profile-messenger', $usereditattempt_data);
			...
			if ( $this->input->is_ajax_request() ) {
				if ( $this->session->userdata('cas-secundaria-userid') && $this->session->userdata('cas-secundaria-username') && $this->session->userdata('cas-secundaria-userdashboard') ) {
					...
					$passwdedit_data = array(
						'editpasswd_userid' => $this->session->userdata('cas-secundaria-userid')
					);
					
					$this->load->view('secundaria/window/window-user-edit-passwd', $passwdedit_data);
				}
				else
					$this->load->view('errors/errors-session-ended');
			}
		...
		public function passwdedit_attempt () {
			if ( $this->input->is_ajax_request() ) {
				if ( $this->session->userdata('cas-secundaria-userid') && $this->session->userdata('cas-secundaria-username') && $this->session->userdata('cas-secundaria-userdashboard') ) {
					...
					$this->load->model('secundaria_users_mdl');
					$userpasswd = $this->secundaria_users_mdl->get_userData_by_ID($this->input->post('userid'), 'user_passwd');
					...
					elseif ( $userpasswd->user_passwd == md5($this->input->post('passwd_actual')) && strlen(trim($this->input->post('passwd_new'))) >= 3 && $this->input->post('passwd_new') == $this->input->post('passwd_renew') ) {
						...
						if ( $this->secundaria_users_mdl->updateUserData($this->input->post('userid'), $passwdeditattempt_info) == 200 ) {
					...
					$this->load->view('secundaria/window/window-user-edit-passwd-messenger', $passwdeditattempt_data);
				}
				else
					$this->load->view('errors/errors-session-ended');
			}
