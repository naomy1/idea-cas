<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	class Secundaria_app extends CI_Controller {
		...
		public function index () {
			if ( $this->input->is_ajax_request() ) {
				if ( $this->session->userdata('cas-secundaria-userid') && $this->session->userdata('cas-secundaria-username') && $this->session->userdata('cas-secundaria-userdashboard') ) {
					...
					$this->load->view('secundaria/app/app-index', $index_data);
		...
		public function app_student () {
			if ( $this->input->is_ajax_request() ) {
				if ( $this->session->userdata('cas-secundaria-userid') && $this->session->userdata('cas-secundaria-username') && $this->session->userdata('cas-secundaria-userdashboard') ) {
					...
					$this->load->model('secundaria_groups_mdl');
					$this->load->model('secundaria_students_mdl');
					$this->load->model('secundaria_apps_mdl');
					
					$active_app = $this->secundaria_apps_mdl->get_active_app();
					...
					$appstudent_data = array(
						...
						'appQuestions'	=> $this->secundaria_apps_mdl->get_app_questions($app_actived_id)
					);
					
					$this->load->view('secundaria/app/app-student', $appstudent_data);
		 	...
		 	if ( $this->input->is_ajax_request() ) {
				if ( $this->session->userdata('cas-secundaria-userid') && $this->session->userdata('cas-secundaria-username') && $this->session->userdata('cas-secundaria-userdashboard') ) {
					...
					$appstudentattempt_info = array(
						...
						'answer_user_id' => $this->session->userdata('cas-secundaria-userid'),
					...
					if ( $this->secundaria_apps_mdl->saveanswers($appstudentattempt_info) ) {
					...
					$this->load->view('secundaria/app/app-student-messenger', $appstudentattempt_data);
				}
				else
					$this->load->view('errors/errors-session-ended');
			}
		  ...
		  public function window_system_locked () {
		  	if ( $this->input->is_ajax_request() ) {
				if ( $this->session->userdata('cas-secundaria-userid') && $this->session->userdata('cas-secundaria-username') && $this->session->userdata('cas-secundaria-userdashboard') ) {
					...
					$this->load->view('secundaria/app/windows/window-system-locked', $systemlocked_data);
				}
				else
					$this->load->view('errors/errors-session-ended');
			}
		   ...
		   public function unlocksystem_attempt () {
		   	if ( $this->input->is_ajax_request() ) {
				if ( $this->session->userdata('cas-secundaria-userid') && $this->session->userdata('cas-secundaria-username') && $this->session->userdata('cas-secundaria-userdashboard') ) {
					...
					$userdata = $this->secundaria_users_mdl->get_userData_by_ID($this->session->userdata('cas-secundaria-userid'), 'user_passwd');
					...
					$this->load->view('secundaria/app/windows/window-system-locked-messenger', $unlocksystem_data);
		   ...
		   public function showschools_attempt () {
		   	if ( $this->input->is_ajax_request() ) {
				if ( $this->session->userdata('cas-secundaria-userid') && $this->session->userdata('cas-secundaria-username') && $this->session->userdata('cas-secundaria-userdashboard') ) {
					...
					$this->load->view('secundaria/app/windows/window-system-locked-continue-messenger', $unlocksystem_data);
