<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	class Preescolar_sessions extends CI_Controller {
		...
		public function login_attempt () {
			...
			if ( $this->input->is_ajax_request() ) {
				...
				$this->load->model('preescolar_sessions_mdl');
				...
				switch ( $validateCode =  $this->preescolar_sessions_mdl->user_validate($username, $password, 1) ) {
					
					// the validation is ok
					case 200 :
						switch ( $sessionCode = $this->preescolar_sessions_mdl->make_session($username) ) {
				...
				$this->load->view('preescolar/form/form-login-messenger', $loginattempt_data);
		...
		public function forgotpasswd_attempt () {
			if ( $this->input->is_ajax_request() ) {
				...
				if ( ($userinfo = $this->preescolar_users_mdl->get_userData_by_email($usermail, 'user_passwd_key, user_name, user_fname, user_lname')) != NULL ) {
				...
				// loading the view
				$this->load->view('preescolar/form/form-forgotpasswd-messenger', $forgotpasswdattempt_data);
			}
			else
				$this->load->view('errors/errors-not-access-allowed');
		}
		...
		public function registeruser_attempt () {
			if ( $this->input->is_ajax_request() ) {
				...
				elseif ( strlen(trim($registeruser_info['user_firstname'])) > 0 && strlen(trim($registeruser_info['user_lastname'])) > 0 && $this->strings_mdl->is_email($registeruser_info['user_useremail']) && $this->strings_mdl->is_curp($registeruser_info['user_usercurp']) && strlen(trim($registeruser_info['user_userusaer'])) > 0 && strlen(trim($registeruser_info['user_userusaerzone'])) > 0 && $registeruser_info['user_usercrosee'] > '0' && $this->strings_mdl->is_username($registeruser_info['user_username']) && strlen(trim($registeruser_info['user_password'])) > 3 && $registeruser_info['user_password'] == $registeruser_info['user_repasswd'] ) {
					$this->load->model('preescolar_users_mdl');
					if ( !$this->preescolar_users_mdl->username_exists($registeruser_info['user_username']) )
						if ( !$this->preescolar_users_mdl->email_exists($registeruser_info['user_useremail']) )
							if ( !$this->preescolar_users_mdl->curp_exists ($registeruser_info['user_usercurp'], 1) )
								if ( $this->preescolar_users_mdl->create_user($registeruser_info) ){
				...
				$this->load->view('preescolar/form/form-registeruser-messenger', $registeruser_data);
