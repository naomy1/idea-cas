<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	class Preescolar_students extends CI_Controller {
		...
		public function index () {
			if ( $this->input->is_ajax_request() ) {
				if ( $this->session->userdata('cas-preescolar-userid') && $this->session->userdata('cas-preescolar-username') && $this->session->userdata('cas-preescolar-userdashboard') ) {
					...
					$this->load->view('preescolar/students/students-index', $group_list_data);
		...
		public function window_addstudent () {
			...
			if ( $this->input->is_ajax_request() ) {
				if ( $this->session->userdata('cas-preescolar-userid') && $this->session->userdata('cas-preescolar-username') && $this->session->userdata('cas-preescolar-userdashboard') ) {
					...
					$this->load->view('preescolar/students/windows/windows-add-student', $addstudent_data);
		...
		public function addstudent_attempt () {
			if ( $this->input->is_ajax_request() ) {
				if ( $this->session->userdata('cas-preescolar-userid') && $this->session->userdata('cas-preescolar-username') && $this->session->userdata('cas-preescolar-userdashboard') ) {
					...
					$this->load->view('preescolar/students/windows/windows-add-student-messenger', $addstudentattempt_data);
		...
		public function deletestudent_attempt () {
			if ( $this->input->is_ajax_request() ) {
				if ( $this->session->userdata('cas-preescolar-userid') && $this->session->userdata('cas-preescolar-username') && $this->session->userdata('cas-preescolar-userdashboard') ) {
					...
					$this->load->model('preescolar_students_mdl');
					$this->preescolar_students_mdl->deletestudent($studentID);
				}
				else
					$this->load->view('errors/errors-session-ended');
			}
		...
		public function window_editstudent () {
			if ( $this->input->is_ajax_request() ) {
				if ( $this->session->userdata('cas-preescolar-userid') && $this->session->userdata('cas-preescolar-username') && $this->session->userdata('cas-preescolar-userdashboard') ) {
					...
					$this->load->view('preescolar/students/windows/windows-editstudent', $editstudent_data);
				}
				else
					$this->load->view('errors/errors-session-ended');
			}
				...
				if ( $this->session->userdata('cas-preescolar-userid') && $this->session->userdata('cas-preescolar-username') && $this->session->userdata('cas-preescolar-userdashboard') ) {
					...
					elseif ( strlen(trim($editstudentattempt_info['student_fname'])) > 0 && strlen(trim($editstudentattempt_info['student_lname'])) > 0 && $this->strings_mdl->is_curp($editstudentattempt_info['student_curp']) ) {
						...
						if ( strtolower($studentInfo->student_curp) == strtolower($editstudentattempt_info['student_curp']) ) {
							if ( $this->preescolar_students_mdl->updateStudent($this->input->post('studentID'), $editstudentattempt_info) ) {
					...
					$this->load->view('preescolar/students/windows/windows-editstudent-messenger', $editstudentattempt_data);
