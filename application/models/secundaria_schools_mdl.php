<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	class Secundaria_schools_mdl extends CI_Model {
		...
		public function getSchoolInfo ( $schoolID = 0, $select = '*' ) {
			...
			return $this->db->get_where('schools', array('school_id' => $schoolID, 'school_level' => 3), 1)->row();
		}
		...
		public function getSchoolInfo_by_cct ( $cct = '', $select = '*' ) {
			$this->db->select($select);
			return $this->db->get_where('schools', array('school_cct' => $cct, 'school_level' => 3), 1)->row();
		}
		...
		public function deleteSchool($schoolID = 0) {
			...
			if ( $schoolInfo->school_user_id == $this->session->userdata('cas-secundaria-userid') ) {
