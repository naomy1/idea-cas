<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	class Preescolar_users_mdl extends CI_Model {
		 ...
		 * @return boolean
		 */
		public function email_exists ($email = '', $level = 1) {
		 ...
		 * @return boolean
		 */
		public function curp_exists ($curp = '', $level = 1) {
		...
		public function create_user ($userdata = array()) {
			...
			if ( !empty($userdata) ){
				...
				if ( $this->db->insert('users',$insert) ) {
					...
					$this->preescolar_sessions_mdl->make_session($insert['user_name']);
		...
		public function getallUsers ($select = '*', $type = 'teacher') {
			$this->db->select($select);
			return $this->db->get_where('users', array('user_school_level' => 1, 'user_type' => $type))->result();
		}
		// end: getAllUsers
		...
		public function get_users ($select = '*') {
			...
			return $this->db->get_where('users', array('user_school_level' => 1))->result();
		}
		// end: get_users
