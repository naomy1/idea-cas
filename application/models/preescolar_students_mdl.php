<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	class Preescolar_students_mdl extends CI_Model {
		...
		public function getPerformanceArea ($student_id = 0) {
			...
			if ( $answers->num_rows() == 1 ) {
				...
				$answer = $answers->row();
				// 91.190
				$linguistico	= $this->answerBitCarry($answer->answer_1) + $this->answerBitCarry($answer->answer_2) + $this->answerBitCarry($answer->answer_3) + $this->answerBitCarry($answer->answer_4);
				$científico		= $this->answerBitCarry($answer->answer_5) + $this->answerBitCarry($answer->answer_6) + $this->answerBitCarry($answer->answer_7) + $this->answerBitCarry($answer->answer_8) + $this->answerBitCarry($answer->answer_9) + $this->answerBitCarry($answer->answer_10) + $this->answerBitCarry($answer->answer_11) + $this->answerBitCarry($answer->answer_12);
				
				
				if ( ($linguistico + ( $científico / 2 )) == 16 ) {
					
					$performanceAreas .= 'L, C';
					$allPerformance++;
					$allPerformance++;
		 ...
		 * answerBitCarry
		 * 
		 * method to transform the answer (1,2,3) to gradual scale (-1,0,1)
		 */
		private function answerBitCarry  ($answer) {
			switch ($answer) {
				case 2: $carry = 0;		break;
				case 3: $carry = 1;		break;
				case 1: default: $carry = -1;
			}
			return $carry;
		}
		// end: answerBitCarry
