<?php
	...
	echo '<span class="history-options">';
		echo '<a href="javascript: contentLoader(\'secundaria_profile_root\', \'students\', \'.dashboard-user-admin\');" class="ho-back">Regresar</a>';
	echo '</span>';
	
	
	echo '<span class="table table-schools-groups-root">';
		...
		echo '<span class="table-header">';
			...
			echo '<span class="header-options">';
				echo '<a href="javascript: void(0);" class="option-item up-grade-groups" id="schoolid_' . $schoolInfo->school_id . '">cambiar de grado todos los grupos</a>';
				echo '<a href="javascript: void(0);" class="option-item addgroup-root" id="schoolid_' . $schoolInfo->school_id . '">agregar grupo</a>';
		...
		if ( sizeof($schoolsGroups) > 0 ) {
			foreach ( $schoolsGroups as $group ) {
				echo '<span class="table-rows' . (($col_counter%2)?'':' odd') . '" id="schoolid_' . $schoolInfo->school_id . '_schoolcct_' . $schoolInfo->school_cct . '_groupid_' . $group->group_id . '">';
					...
					echo '<span class="col-title col-identifier col-groups">' . $this->secundaria_groups_mdl->groupStudentsCounter($group->group_id) . '</span>';
					echo '<span class="clear"></span>';
				echo '</span>';
				$col_counter++;
			}
